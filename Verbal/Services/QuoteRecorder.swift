...
final class QuoteRecorder {
    ...
    func start() async {
        ...
        do {
            guard let locale = await Self.transcriptionLocale() else {
                errorMessage = "Speech recognition isn't available in this language yet. Pick another under Settings → Dictation language."
                state = .unavailable
                return
            }
    ...
    /// The locale to transcribe in. The choice itself lives in
    /// `DictationLanguage`, which Settings writes and this reads, so the
    /// language the user picked is the language the mic hears.
    private static func transcriptionLocale() async -> Locale? {
        await DictationLanguage.resolved()
    }
