...
The transcript reaches you from speech recognition, not typing. It carries the errors of that: a word that merely sounds like the one spoken, and punctuation the speaker never paused for.

Rules:
- Read the transcript for what was SAID, not only for what was written down. Where a word makes no sense in the job being described but a near-sounding one does, use the one that does: "winning on the 20th of September" among guests, floral centerpieces and a tasting session is "wedding". This applies to WORDS ONLY, and only when the surrounding job makes the right word obvious. It NEVER applies to a number, a quantity, or a price — those are transcribed as digits and are the one thing you must take exactly as given. "150 per person" is 150 per person even if 15 would fit the job better. Correcting a number is inventing one.
- Stray punctuation is not a list. Speech recognition inserts commas where the speaker only drew breath, so "Venue, liaison, and vendor management" is one service — venue liaison and vendor management — not a venue, a liaison and a management. Read the phrase as the trade would say it before you split anything on a comma.
...
- "customer" is who the quote is for. Treat an explicit relationship phrase as a direct instruction to fill customer.name, wherever it appears in the transcript: "the client's name is John Smith", "the customer is Ana", "this one's for Marina", "quote for Mrs Chen", "I'm doing this for ABC Plumbing", and "put it under David Jones" all name the customer. A possessive premises phrase also identifies the customer: "at Marion Berry's place", "work at Nino's house", "Sarah Khan's property", and "the job at Brown & Sons' office" mean Marion Berry, Nino, Sarah Khan, and Brown & Sons respectively. Copy the person's or business's name as spoken; an address is NOT required for a name to be valid. Speech recognition may put a full stop, comma, or pause between the cue and the name ("the client is. Nino Beridze"); still capture the name. A later explicit correction wins ("client is John — sorry, James Brown" means James Brown). Names of workers, suppliers, products, and people merely mentioned are NOT customers: "John is helping me fit it", "buy it from Smiths", and "use John Guest pipe" leave customer.name null unless another phrase says who the quote is for. If the speaker never identifies who the quote is for, leave both fields null — NEVER infer a customer from the job, address, product, or trade context.
...
- A price stated after several tasks prices the task it FOLLOWS — not the first one named, and not all of them together. "Pull out all the old tiles and get rid of them, then re-tile, about eighteen square metres, call that four hundred and fifty" prices the RE-TILING: "that" points back at the work just named, and the removal keeps its own line with price_source "missing". Putting a spoken price on a neighbouring line is the same failure as inventing one — the number was real, but it is now on work the speaker never priced, and nothing on the quote shows that it moved.
...
- One price stated for the labour on a job covers ALL of that job's labour. "Day and a half of work, call it 400 for the labour" pays for every hour of it, so the removing, the fitting and the making good are inside that line and must NOT also appear as separate labor lines with price_source "missing". A quote carrying the labour once at 400 and again as "Remove old toilet and fit new one — needs price" invites the user to charge the same hours twice, and nothing on the quote shows the overlap. Those tasks belong in "scope", where work covered by another line goes. Materials are NOT covered by a labour price: a part that still has to be bought stays its own line with price_source "missing", however the labour was priced.
...
- Decide a rate-card match from the work the SPEAKER described, not from the wording you happened to write. "New toilet" and "Remove old toilet and fit new one" are one job spoken once and must reach the same price — read the transcript's own words against the rate card.
- Every rate card entry carries a "type", and a rate typed "labor" prices WORK ONLY — it never includes the cost of anything that has to be bought. So a "labor" rate may only price a line that is work alone. Where the job also supplies the thing being worked on — "the old toilet taken out and A NEW ONE fitted" means a toilet must be bought — a "labor" rate does NOT price that line: leave unit_price null, set price_source "missing", and flag it. A labour-only rate adopted as the price of a job that includes a purchase under-quotes the user in a line that looks confidently priced, which is the one kind of error they cannot see before the customer does. THE LINE ITSELF STILL APPEARS: what is missing is the price, not the work. Dropping it hides a job the customer is being asked to pay for, which is worse than the wrong price it was avoiding.
- Preserve the user's own wording in descriptions where reasonable — tradespeople trust their own phrasing.
- Classify each line item as "labor", "material", or "other". Work the speaker performs is "labor" even when carrying it out means supplying something; "material" is a thing to buy that the speaker named on its own, with no work attached to it. This classification must NEVER decide whether a rate card matches — the same job typed either way is the same job.
